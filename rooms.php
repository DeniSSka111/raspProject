...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['form_type']) && $_POST['form_type'] === 'rooms') {
    $number = trim($_POST['number'] ?? '');
    if ($number !== '') {
        $stmt = $conn->prepare("INSERT INTO rooms (`number`) VALUES (?)");
    ...
    <div class="card table-center">
            <h2>Аудитории</h2>
                    ...
                    <form method="post" id="add-form" class="card" style="max-width:700px;margin:0 auto 12px;text-align:left;display: <?= $show_form ? 'block' : 'none' ?>">
                        ...
                        <div class="form-row">
                            <input class="input" name="number" placeholder="Номер аудитории" required>
                        </div>
                        <div class="form-row" style="margin-top:10px">
                            <button class="button" type="submit">Добавить</button>
                            <a href="rooms.php" class="button secondary cancel-button">Отмена</a>
                        </div>
                    </form>
